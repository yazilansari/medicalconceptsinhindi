...
class Usertracking
{
	...
	public function get_logged_in_user($tracking_data)
	{
		...
		$admin = $this->CI->session->userdata('masterAdminData');
		if (!empty($admin['user_id'])) {
			return (is_numeric($admin['user_id'])) ? $admin['user_id'] : 0;
		}
	}
	...
	public function track_this()
	{
		...
		$input_data['user_identifier'] = $this->get_logged_in_user($input_data);
