...
class Contributors extends Generic_Controller
{
	...
	function __construct() {
		...
		$this->data['columns'] = ['Contributors Name','Contributors Designation','Contributors Image','Contributors Data'];
	...
	function options_new(){
		...
		foreach($_options as $option){
			$new = [ 'id' => $option->id, 'text' => $option->name	];
			array_push($json['results'], $new);
		}
	...
	function lists_new(){
		...
		if(!empty($this->data['collection'])){
			...
			foreach ($this->data['collection'] as $key => $value) {
				...
				$this->data['collection'][$key]->image = $this->model->get_image_path($value->image,$this->config->item('contributors_images_path_list'));			
			}
		}
