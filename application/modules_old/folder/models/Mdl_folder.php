...
class Mdl_folder extends MY_Model {
	...
	function get_collection($f_filters = [], $keywords ='', $limit = 0, $offset = 0 ) {
    	...
    	$q = $this->db->select('f.folder_name,f.folder_id,f.thumbnail,mc.main_category_id,mc.main_category_name,c.category_id,c.category_name')
		->from('folder f')
...
//var_dump($collection);exit;
	...
	function save(){
		...
		$uploadPath = $s3uploadPath = "folder/thumbnail/$folder_id/";

		if($_FILES['thumbnail']['size'] > 0) {
            	if(!empty($_FILES['thumbnail']['name'])){
                $img_response = $this->file_upload($uploadPath, 'thumbnail', $_FILES['thumbnail']['name']);
                $data['thumbnail'] = $thumbnail = $img_response['u_response']['full_path'];
              	$name = $img_response['u_response']['filename'];
              	$filename = base_url().'uploads/'.$uploadPath.$name;
				$folder_image = 'uploads/'.$uploadPath.$name;
              	//var_dump($folder_image,$filename);
				$data = array(
			        'thumbnail'=>$folder_image
			    );
