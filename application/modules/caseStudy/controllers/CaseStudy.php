...
class CaseStudy extends Generic_Controller
{
	private $module = 'caseStudy';
	private $model_name = 'mdl_casestudy';
	...
	function lists(){
		...
		$where = array("(ud.upload_type='text' OR ud.upload_type='image')" => NULL, "cat.category_name LIKE '%Case Study%'" => NULL);
		$keywords = !empty($this->input->post('keywords'))?$this->input->post('keywords'):'';

		if (!empty($keywords)) {
			
			$this->data['collection'] = $this->model->get_collection($sfilters, $keywords, $where, $this->perPage);
			
		} else {
			
			$this->data['collection'] = $this->model->get_collection($sfilters, $keywords, $where, $this->perPage, $page);
		}

		//echo "<pre>";print_r($this->data['collection']);echo "</pre>";exit;
	
		$totalRec = count($this->model->get_collection($sfilters, $keywords, $where));
