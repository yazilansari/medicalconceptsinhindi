...
class Mdl_category extends MY_Model {
	...
	public $p_key_new = 'id';
	public $table_new = 'mch_categories';
	...
	function get_collections_new($f_filters = [], $keywords ='', $limit = 0, $offset = 0 ) {
    	...
    	$q = $this->db->select('
    		mch_categories.id, mch_categories.name AS category_name, mch_parent_categories.name AS parent_category_name, mch_categories.parent_category_id AS parent_category_id
    	')
		->from('mch_categories')
		->join('mch_parent_categories','mch_parent_categories.id = mch_categories.parent_category_id');
		...
		if(!empty($keywords)) { 
			...
			$where_condition = "(
				mch_categories.name like '%". $s_key ."%'
			) ";

			$q->where($where_condition, NULL, FALSE);
		}

		$q->where("mch_categories.is_active","1");
		$q->order_by('mch_categories.id desc');
	...
	function is_a_unique_category_new( $category_name = '' ){
		if(empty($category_name)){
			return TRUE;
		}
		if(!empty($category_name)){
	...
	function save_new(){
		...
		$category_name = trim($this->input->post('category_name'));
		$parent_category_id = trim($this->input->post('main_category_id'));

		$valid_category = 1;
		if($_POST['main_category_id']!='' && $_POST['category_name']!=''){
			$check_data = $this->get_collections_new(['mch_categories.name' => $_POST['category_name']]);	
			// print_r($check_data);
			if(!empty($check_data) && $check_data[0]->category_name==$_POST['category_name'] && $check_data[0]->parent_category_id==$_POST['main_category_id']){
				$valid_category=0;
			}
		}
		...
		if(!$this->form_validation->run() || $valid_category!=1){
			...
			// $map_data = array();
			// $map_data['parent_category_id'] =  !empty($this->input->post('main_category_id'))?$this->input->post('main_category_id'):NULL;
			// $map_data['category_id'] =  $category_id;
			// $mapping_category_id = $this->_insert($map_data,'main_category_mapping');	
	...
	function modify_new(){
		...
		if(! $this->form_validation->run() || $valid_category!=1){
			...
			$status = (int) $this->_update([$p_key_new => $category_id], $data, $this->table_new);
			$response['status'] = ($status) ? TRUE : FALSE;
			$response['redirect'] = 'lists_new';
		}
		return $response;
	...
	function remove_new(){
		...
		if(isset($_POST['ids']) && sizeof($_POST['ids']) > 0){
			...
			$where_condition = array("category_id IN ({$ids1})" => NULL ,"is_active" => "1");
			$category_records = $this->get_records($where_condition, 'mch_sub_categories', ['GROUP_CONCAT(category_id) category_id']);
			...
			if(!empty($category_ids)){
				...
				$data['is_active'] = "0";
				$response = $this->_update_with($this->p_key_new, $category_ids, array(), $data, $this->table_new);
