...
<tr id="tr_<?php echo $id;?>">
	...
	<td><?php if($record->upload_type=='image'){?>
			<a class="fancy" id="single_image" href="<?php echo $this->config->item('posts_images_path').$record->sub_category_id.'/'.$record->upload_path ?>"><img src="<?php echo $this->config->item('posts_images_path').$record->sub_category_id.'/'.$record->upload_path ?>" style="width: 50px; height: 50px;" alt=""/></a>
		<?php }else if($record->upload_type=='video'){?>
			<?php if($record->video_type=='inhouse' || $record->video_type==''){?>
				<a data-fancybox href="<?php echo $this->config->item('posts_video_path').$record->sub_category_id.'/'.$record->upload_path ?>">
		...
		<?php }elseif($record->upload_type=='pdf'){?>
			<a data-fancybox href="<?php echo $this->config->item('posts_pdf_path').$record->sub_category_id.'/'.$record->upload_path ?>">
			    <?php echo $record->upload_path;?>
			</a>	
		<?php }else?>
	</td>
