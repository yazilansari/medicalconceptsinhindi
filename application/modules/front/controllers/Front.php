...
class Front extends Generic_Controller
{
	...
	function index(){
		...
		$this->data['text_post'] = $this->model->get_post_collection(['sc.is_active' => '1','ud.is_active' => '1','ud.upload_type' => 'text'], '',3);
		
		//echo "<pre>";print_r($this->data);echo "</pre>";exit;
		$this->data['module'] = 'front';
		$this->data['plugins'] = ['front_paginate'];
		$title_txt = 'MCH - Medical Concepts in Hindi By Dr Pankaj Agarwal';

		$this->set_view($this->data, 'home',  '_front', $title_txt);
	}
	...
	function save(){
        $recaptchaResponse = trim($this->input->post('g_recaptcha_response'));
		$userIp = $this->input->ip_address();
		$secret = "6LcC5EMaAAAAADuhma1O0DwNtM1KYid0E6tqEsi4";
