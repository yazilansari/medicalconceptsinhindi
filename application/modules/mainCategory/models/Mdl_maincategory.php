...
class Mdl_maincategory extends MY_Model {
	...
	function getfolders($dbcatID){
	    if(!empty($dbcatID)){
	        $this->db->order_by('folder_name','asc');
	        $query= $this->db->get_where('folder',array('category_id'=>$dbcatID,'is_active'=>1));
	...
	function getfoldersubcategories($folder_id,$category_id,$limit = 0,$offset = 0){
	    if(!empty($category_id)){
	        $query= $this->db->select('sub_category.*');
	        $this->db->from('sub_category');
	        $this->db->where(array('category_id'=>$category_id,'folder_id'=>$folder_id,'is_active'=>1));
	...
	function getsubcategory($category_id,$sub_category_id,$limit = 0,$offset = 0){
	    if(!empty($category_id) && !empty($sub_category_id)){
	        
	        $query= $this->db->select('ud.*,md.*');
            $this->db->from('upload_data ud');
            $this->db->join('meta_tag_details md', 'md.meta_upload_data_id = ud.upload_data_id and md.is_active = 1','left');
            $this->db->where(array("ud.category_id"=>$category_id,"ud.sub_category_id"=>$sub_category_id,"ud.is_active"=>1));
            $this->db->order_by('ud.upload_data_id', 'desc');
