...
class Ajax_pagination{
   ...
   var $anchor_class_grid	= 'grid-bullets';
   var $show_count      = true;
	/**
	 * Constructor
	 *
	...
	function initialize($params = array())
	{
		...
		if ($this->anchor_class_grid == 'grid-bullets')
		{
			$this->anchor_class_grid = 'class="waves-effect ' . $this->anchor_class_grid . '" ';
		}
	}
	...
	function create_links_grid()
	{
      ...
      if ($this->show_count)
      {
		...
		{
			$output .= $this->first_tag_open
					. $this->getAJAXlink_grid( '' , $this->first_link)
					. $this->first_tag_close;
		}
		// Render the "previous" link
		if  ($this->cur_page != 1)
		{
			...
			if ($i == 0) $i = '';
			$output .= $this->prev_tag_open
					. $this->getAJAXlink_grid( $i, $this->prev_link )
					. $this->prev_tag_close;
		}
		// Write the digit links
		for ($loop = $start -1; $loop <= $end; $loop++)
		...
		{
			$output .= $this->next_tag_open
				. $this->getAJAXlink_grid( $this->cur_page * $this->per_page , $this->next_link )
				. $this->next_tag_close;
		}
		// Render the "Last" link
		if (($this->cur_page + $this->num_links) < $num_pages)
		{
			$i = (($num_pages * $this->per_page) - $this->per_page);
			$output .= $this->last_tag_open . $this->getAJAXlink_grid( $i, $this->last_link ) . $this->last_tag_close;
		}
		// Kill double slashes.  Note: Sometimes we can end up with a double slash
		// in the penultimate link so we'll kill all double slashes.
		$output = preg_replace("#([^:])//+#", "\\1/", $output);
	...
	function getAJAXlink_grid( $count, $text) {
        if( $this->div == '')
            return '<a href="'. $this->anchor_class_grid . ' ' . $this->base_url . $count . '">'. $text .'</a>';
		...
		return "<a href=\"javascript:void(0);\"
		         " . $this->anchor_class_grid . " data-count=\"$count\" data-page=\"$pageCount\" >"
				. $text .'</a>';
	}
}
