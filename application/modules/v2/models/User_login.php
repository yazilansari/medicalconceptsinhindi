...
class User_login extends CI_Model
{
    ...
    public function get_upcomming_event_data($start_from = "", $limit = "")
    {
        ...
        $q = $this->db->query("SELECT * FROM upload_data WHERE is_active='1' && event_date >='$added_date' order by upload_data_id DESC LIMIT $start_from,$limit");
        if ($q->num_rows() > 0) {
            $data = $q->result_array();
    ...
    public function get_past_event_data($start_from = "", $limit = "")
    {
        ...
        $q = $this->db->query("SELECT * FROM upload_data WHERE is_active='1' && category_id ='18' && event_date < '$currentdate' order by upload_data_id DESC LIMIT $start_from,$limit");
        if ($q->num_rows() > 0) {
            $data = $q->result_array();
    ...
    public function get_all_event_data($start_from = "", $limit = "")
    {
        ...
        $q = $this->db->query("SELECT * FROM upload_data WHERE is_active='1' && category_id ='18' order by upload_data_id DESC LIMIT $start_from,$limit");
        if ($q->num_rows() > 0) {
            $data = $q->result_array();
