...
if (isset($_POST['submit'])) {
    ...
    //     exit();
    // } else {
        $num = "INSERT INTO `mch_contact`(`name`, `email`, `number`, `message`, `created_at`) VALUES ('$name', '$email','$mobile', '$message', '".date('Y-m-d H:i:s')."')";
        // echo $num;die();
        $num_final = mysqli_query($conn, $num);

        if ($num_final) {
            echo json_encode(array('status' => 1, 'msg' => 'Data Inserted Successfully.'));
            // $_SESSION['reg_id'] = mysqli_insert_id($conn);
            // if($is_referral_code == 'No') {
                // $_SESSION['type'] = $type;
                // header("Location: ../index.php?msg=Contact Successfully Added");
            // } else {
            ...
            echo json_encode(array('status' => 0, 'msg' => 'Error While Submitting.'));
            // header("Location: ../index.php?msg=Contact Error");
            exit();
        }
    // }
} else {
