...
class Login extends MY_Controller
{
	...
	public function register()
	{
		...
		$flag = true;
		$message = "";
		$password 		= isset($data['password']) ? $data['password'] : "123";
		...
		if ($flag) {
			...
			if (empty($result_data)) {
				...
				if (strtolower($user_type) == "general") {
				...
				$this->response['code'] = 400;
				$this->response['message'] = "User account already exist!!";
			}
	...
	public function get_contributors()
	{
		...
		if ($flag) {
			...
			foreach ($result_data as $key => $value) {
				...
				if (!empty($value->contributors_image)) {
					$result_arr[$key]['contributors_image'] = 'http://medicalconceptsinhindi.in/uploads/assets/uploaded_data/contributors_img/' . $value->contributors_image;
				} else {
					$result_arr[$key]['contributors_image'] = '';
				}


				/*get_contributors_image_path($value->contributors_image);*/
	...
	public function contributors_details()
	{
		...
		if ($flag) {
			...
			$result_arr['contributors_image'] = "http://medicalconceptsinhindi.in/uploads/assets/uploaded_data/contributors_img/" . $result_data->contributors_image;
	...
	public function switch_usertype()
	{
		...
		if ($usertype == "") {
			$flag = false;
			$this->error = array("message" => "Please enter usertype");
		...
		if ($flag) {
			...
			$result_data = $this->login_model->switch_usertype($userid, $usertype);
			if ($result_data === true) {

				$this->response['code'] = 200;
				$this->response['message'] = "user type changed,please logout";
				$this->response['data'] = $result_data;
			} else {

				$this->response['code'] = 200;
				$this->response['message'] = "Error changing user type";
				$this->error['message'] = array("message" => "");
			}
		} else {
			$this->response['code'] = 404;
