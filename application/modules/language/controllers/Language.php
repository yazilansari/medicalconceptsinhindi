...
class Language extends Generic_Controller
{
	...
	function __construct() {
		...
		$this->data['mainmenu'] = $this->module;
		$this->data['controller'] = $this->data['menu'] = $this->data['m_title'] = $this->module;
		$this->data['columns'] = $this->csv_fields = ['Language Name','Language Code'];
	}
	...
	function options(){
		...
		$_options = $this->model->get_options($s_term, 'language_name', [], $page * $limit, $limit);
		$_opt_count = count($this->model->get_options($s_term, 'language_name'));

		foreach($_options as $option){
			$new['id'] = $option->language_code;
			$new['text'] = $option->language_name;
	...
	function uploadcsv(){
		...
		while (($data = fgetcsv($handle, 1000, ",")) !== FALSE){
			...
			if($cnt != 0){
				$language_name = trim($data[0]);
				$language_code = trim($data[1]);

				if( empty($language_name) ){
					continue;
				}

				if( !preg_match('/^[a-zA-Z][a-zA-Z0-9 \.]+$/', $language_name) ){
					continue;
				}
				...
				if( !preg_match('/^[a-zA-Z0-9 \.]+$/', $language_code) ){
					continue;
				}

				$is_exist_name = $this->model->get_records([ 'language_name'=> $language_name ], 'language', ['language_id'], '', 1);
				$is_exist_code = $this->model->get_records([ 'language_code'=> $language_code ], 'language', ['language_id'], '', 1);

				if(count($is_exist_name) || count($is_exist_code)){
					continue;
				}

				$insert['language_name'] = $language_name;
				$insert['language_code'] = $language_code;
